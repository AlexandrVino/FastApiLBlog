...
async function registerUser(email, password) {
    const data = await http('/api/v1/auth/register', {method: 'POST', body: {email, password}, auth: false});
    setAuth(data);
    return data;
}

async function loginUser(email, password) {
    const data = await http('/api/v1/auth/login', {method: 'POST', body: {email, password}, auth: false});
    setAuth(data);
    return data;
}
...
async function refreshToken() {
    try {
        const data = await http('/api/v1/auth/refresh', {method: 'POST', auth: false});
        setAuth(data);
        return true;
    } catch (e) {
        clearAuth();
...
// Users (admin)
async function adminListUsers(page = 0, pageSize = 50) {
    return http(`/api/v1/users/admin/?page=${page}&page_size=${pageSize}`);
}

async function adminGetUser(id) {
    return http(`/api/v1/users/admin/${id}`);
}

async function adminUpdateUser(id, role) {
    return http(`/api/v1/users/admin/${id}`, {method: 'PUT', body: {role}});
}

async function adminDeleteUser(id) {
    return http(`/api/v1/users/admin/${id}`, {method: 'DELETE'});
}
...
async function adminCreatePost(payload) {
    return http('/api/v1/admin/posts/', {method: 'POST', body: payload});
}
...
async function adminUpdatePost(id, payload) {
    return http(`/api/v1/admin/posts/${id}`, {method: 'PUT', body: payload});
}

async function adminDeletePost(id) {
    return http(`/api/v1/admin/posts/${id}`, {method: 'DELETE'});
}
...
async function adminCreateCategory(payload) {
    return http('/api/v1/admin/categories/', {method: 'POST', body: payload});
}
...
async function adminUpdateCategory(id, payload) {
    return http(`/api/v1/admin/categories/${id}`, {method: 'PUT', body: payload});
}

async function adminDeleteCategory(id) {
    return http(`/api/v1/admin/categories/${id}`, {method: 'DELETE'});
}
